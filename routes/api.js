...
(function(){
	...
	.put('/project.json/:_id', function(req, res){
		cPro.update({_id: req.params._id}, req.body, function(err, data){
			res.json(data);
		});
	})
	.delete('/project.json/:_id', function(req, res){
		cPro.remove({_id: req.params._id}, function(err, data){
			if(err) throw err;
			cMod.remove({pid: req.params._id});
			cInt.remove({pid: req.params._id}, function(err, data){
				if(err) throw err;
				else http.get(conf.mockUrl+_.now());
			});
			res.json(data);
		});
	})
	...
	.get('/modules.json/:pid', function(req, res){	
		...
		function gm(pId){
			var def = q.defer();
			cMod.find({pid: pId}, function(err, data){
		...
		function gi(pId){
			var def = q.defer();
			cInt.find({pid: pId}, {sort: {mid: 1}}, function(err, data){
				if(err) def.reject(err);
				else def.resolve(data||[]);
			});
			return def.promise;
		...
		q.all([gm(pId), gi(pId)]).then(function(result){
			...
			_.each(modules, function(mod){
				mod.interfaces = _.filter(interfaces, function(face){return face.mid===mod._id.toString();});
			});
			res.json(modules);
		}, function(err){
			throw err;
	...
	.put('/module.json/:_id', function(req, res){
		cMod.update({_id:req.params._id}, req.body, function(err, data){
			if(err) throw err;
			else res.json(data);
		});
	})
	.delete('/module.json/:_id', function(req, res){
		cMod.remove({_id: req.params._id}, function(err, data){
			if(err) {
				throw err;
			} else {
				res.json(data);
				cInt.remove({mid: req.params._id}, function(err, data){
					if(err) throw err;
					else http.get(conf.mockUrl+_.now());
				});
			}
	...
	.get('/interface.json/:_id', function(req, res){
		cInt.find({_id: req.params._id}, function(err, data){
			if(err) throw err;
			console.log(data);
	...
	.post('/interface.json', function(req, res){
		cInt.insert(req.body, function(err, data){
			if(err) throw err;
			http.get(conf.mockUrl+_.now());
			res.json(data);
		});
	})
	.put('/interface.json/:_id', function(req, res){
		cInt.update({_id:req.params._id}, req.body, function(err, data){
			http.get(conf.mockUrl+_.now());
			res.json(data);
		});
	})
	.delete('/interface.json/:_id', function(req, res){
		cInt.remove({_id: req.params._id}, function(err, data){
			if(err) throw err;
			http.get(conf.mockUrl+_.now());
			res.json(data);
		});
	...
	.post('/login.json', function(req, res){
		var self = this;
		cUsr.find({name: req.body.name, password: req.body.password}, function(err, data){
			if(err){
				throw err;
			} else if(data.length){
				req.session.user = data[0];
				res.json({url:'/api/projects.html'});
			} else {
	...
	.get('/users.json', function(req, res){
		if(req.session.user.manageable){
			cUsr.find({name:{$ne:'admin'}}, function(err, data){
				if(err) throw err;
				else res.json(data);
			});
		} else {
	...
	.put('/self.json', function(req, res){
		cUsr.find({_id: req.body._id, password: req.body.password}, function(err, data){
			if(err){
				throw err;
			} else if(data.length){
				cUsr.update({_id: req.body._id}, {password: req.body._password}, function(err, data){
					if(err) throw err;
					else res.json({status: data, message: '修改成功'});
				});
			} else {
				res.json({status: 0, message: '密码错误，无法修改'});
			}
		});
	})
	.put('/user.json/:_id', function(req, res){
		cUsr.update({_id:req.params._id}, req.body, function(err, data){
			if(err) throw err;
			else res.json(data);
		});
	})
	.post('/user.json', function(req, res){
		cUsr.find({name: req.body.name}, function(err, data){
			if(err) throw err;
				...
				res.json({status: 0, message:'用户名已被占用！'});
			}
		});
	})
	.delete('/user.json/:_id', function(req, res){
		cUsr.remove({_id: req.params._id}, function(err, data){
			if(err) throw err;
			else res.json(data);
		});
	})
	;
	//创建管理员用户
	var admin = {name: 'admin', password: 'admin', manageable: true, editable: true};
	cUsr.find({name:'admin'}, function(err, data){
		if(err){
			throw err;
		} else if(data.length===0){
			cUsr.insert(admin);
